...
export default function App() {
    ...
    const unsubscribeAuth = api.onAuthChange(async (currentUser) => {
      ...
      if (currentUser) {
        try {
          ...
          if (!profile) {
            await api.createUserProfile(currentUser.id, currentUser.email || "");
          }
  ...
  if (!isSupabaseConfigured) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center font-mono p-8 text-center">
        ...
        <h1 className="text-2xl font-black uppercase mb-4 tracking-tighter text-red-500">CONFIGURATION ERROR</h1>
        <div className="max-w-md bg-zinc-900 border border-zinc-800 p-6 rounded-lg text-left space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Status Check:</p>
            <ul className="text-[10px] space-y-1 font-mono">
              <li className="flex items-center gap-2">
                <span className="text-red-500">
                  [MISSING]
                </span>
                <code className="bg-black px-1 py-0.5 rounded text-zinc-300">VITE_SUPABASE_URL</code>
              </li>
              <li className="flex items-center gap-2">
                <span className="text-red-500">
                  [MISSING]
                </span>
                <code className="bg-black px-1 py-0.5 rounded text-zinc-300">VITE_SUPABASE_ANON_KEY</code>
              </li>
            </ul>
          </div>
          <div className="pt-4 border-t border-zinc-800">
            <p className="text-[10px] text-zinc-500 mb-2 uppercase font-bold">How to fix in Netlify:</p>
            <ol className="text-[10px] text-zinc-400 list-decimal list-inside space-y-1 uppercase tracking-tighter">
              <li>Open <span className="text-white">Site configuration</span> &gt; <span className="text-white">Environment variables</span></li>
              <li>Add both variables listed above</li>
              <li>Go to <span className="text-white">Deploys</span> tab</li>
              <li>Click <span className="text-white">Trigger deploy</span> &gt; <span className="text-white">Clear cache and deploy site</span></li>
            </ol>
          </div>
        </div>
        <button 
  ...
  return (
    <div className={cn(
        ...
        isRedMode && "bg-red-950/20 text-red-500 selection:bg-red-500 selection:text-black border-red-500/20",
